...
function registerHandlerForUpdateCurrentPriceAndFeed() {
    var eventBus = new EventBus('http://localhost:8084/eventbus');
    eventBus.onopen = function () {
        eventBus.registerHandler('auction', function (error, message) {
...
function sendData(){
    var eventBus = new EventBus('http://localhost:8084/eventbus');
    eventBus.onopen = function () {
     eventBus.send('auction',JSON.stringify({'price': '0007'}));
           /*eventBus.registerHandler('send',  function (error, message) {
                            alert('Registering Habdlers....');

                    });*/
}
      /*alert('Sending Data....');
      eventBus.send('http://localhost:8084/eventdata/send','Ok Data....');*/
     //}
